...
const gauthOption = {
  clientId: '243699286858-d5mocb3bti7a8mg9f717m3b1dgc60dku.apps.googleusercontent.com',
  scope: 'profile email',
  ...
  methods: {
    ...
    handleClickSignIn(){
      ...
      .then(async user => {
        ...
        var postResult = await PostService.addUserPost(userInfo).then(
          response => {

                this.$store.commit('setUserInfo', response.data.userEmail)
